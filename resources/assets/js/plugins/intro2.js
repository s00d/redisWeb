export function install(Vue) {
    Vue.prototype.$runIntro = (step) => {
        ...
        if(typeof step == 'undefined') intro.start();
        else intro.goToStep(step).start();
    };
}
...
function _showElement(targetElement) {
    ...
    if (oldHelperLayer != null) {
        ...
        //myLabel button
        if (this._options.myLabel) {
            var myButton = document.createElement('a');
            myButton.className = 'introjs-button introjs-mybutton';
            myButton.href = 'javascript:void(0);';
            myButton.innerHTML = this._options.myLabel;

            myButton.onclick = function() {
                // notify('info','Для получения справки наведите на элемент', 'info');

                introJs().setOption('tooltipClass', 'introjs-tooltip-hint').addHints();
            ...
            buttonsLayer.appendChild(myButton);
        }
...
var ondclick = null;
function hideAllBodyHint(e) {
    e = e || window.event;
    if (e.stopPropagation) e.stopPropagation();
    // alert("You clicked outside the area!");
    var hints = document.querySelectorAll('.introjs-hint');
    if (hints && hints.length > 0) {
        for (var item of hints) item.parentNode.removeChild(item);
    }
    for (var item of document.getElementsByClassName("introjs-tooltip-hint")) item.remove();

    document.body.onmousedown = ondclick
}
...
function _addHints() {
    ...
    ondclick = document.body.onmousedown
    document.body.onmousedown = hideAllBodyHint
    ...
    for (var i = 0, l = this._introItems.length; i < l; i++) {
        ...
        hint.onmouseleave = function(e) {
            _hideHint.call(self, i);
        };
